...
function MessageInput({ isPanelTop, placeholder, Addtext, onClearText, onSend, addFiles }) {
    const { theme } = useContext(ElementContext);
    const inputBodyRef = useRef(null);
    const contentEditableRef = useRef(null);
    ...
    // Обработка добавления текста извне
    useEffect(() => {
        if (Addtext) {
            setText(Addtext + '\u200B');
        }
    ...
    // Обработка изменений текста и обновление высоты
    useEffect(() => {
        if (text === '') {
            onClearText && onClearText();
            inputBodyRef.current.style.height = 'auto';
        ...
        if (contentHeight > 40 && isPanelTop) {
            setMarginTop(contentHeight > 90 ? 120 : contentHeight + 20);
        }
    }, [text, onClearText, isPanelTop]);

    // Обработка количества изображений и изменение marginTop
    useEffect(() => {
        if (images.length > 0) {
            setMarginTop(160);
            return;
    ...
    // Очистка URL.createObjectURL при удалении изображений
    useEffect(() => {
        return () => {
    ...
    const addEmoji = (emoji, isImg, font) => {
        if (isImg) {
            const imgEmoji = `<img class="${styles['emoji-img']}" src="${emoji}" style="width: 22px; height: 22px"/>`;
            setText(text + imgEmoji + '\u200B');
            return;
        }
    ...
    return (
        <>
            <section className={styles['input-container']}>
                ...
                <div 
                    className={styles['input-message-body']}
                    style={{ background: theme.element_first_color }}
                >
                    {images.length > 0 && (   
                        <div className={styles['image-previewer']}>
                            {images.map((image, index) => (
                                ...
                                >
                                    ...
                                    <div className={styles['image-no-select']}></div>
                                    {image && <img src={image.preview || URL.createObjectURL(image)} alt="Preview"/>}
                                </div>
                            ))}
                        </div>
                    )}
                    ...
                    <div 
                        className={styles['input-message-wrapper']}
                        ref={inputBodyRef}
                        style={{ alignItems: isPanelTop ? 'flex-end' : 'flex-start' }}
                    >
                        <button 
                        ...
                        >
                            <svg width="22" height="22" viewBox="0 0 22 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M11 21C16.5228 21 21 16.5228 21 11C21 5.47715 16.5228 1 11 1C5.47715 1 1 5.47715 1 11C1 16.5228 5.47715 21 11 21Z" stroke={theme.text_first_color} strokeWidth="2"/>
                                <path d="M8.25049 15.5C9.13085 16.1525 10.2028 16.5357 11.3576 16.5357C12.5125 16.5357 13.5844 16.1525 14.4648 15.5" stroke={theme.text_first_color} strokeWidth="2" strokeLinecap="round"/>
                                <path d="M15.5006 9.80356C15.5006 10.6611 15.0366 11.3571 14.4649 11.3571C13.8932 11.3571 13.4292 10.6611 13.4292 9.80356C13.4292 8.94599 13.8932 8.24999 14.4649 8.24999C15.0366 8.24999 15.5006 8.94599 15.5006 9.80356Z" fill={theme.text_first_color}/>
                                <path d="M8.25007 11.3571C8.82208 11.3571 9.28578 10.6616 9.28578 9.80356C9.28578 8.94555 8.82208 8.24999 8.25007 8.24999C7.67806 8.24999 7.21436 8.94555 7.21436 9.80356C7.21436 10.6616 7.67806 11.3571 8.25007 11.3571Z" fill={theme.text_first_color}/>
                            </svg>
                        </button>
                        ...
                        >
                            <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M2.20711 11.2071C1.81658 11.5976 1.18342 11.5976 0.792893 11.2071C0.402369 10.8166 0.402369 10.1834 0.792893 9.79289L2.20711 11.2071ZM10.5 0.5C11.0523 0.5 11.5 0.947715 11.5 1.5L11.5 10.5C11.5 11.0523 11.0523 11.5 10.5 11.5C9.94772 11.5 9.5 11.0523 9.5 10.5L9.5 2.5L1.5 2.5C0.947715 2.5 0.5 2.05228 0.5 1.5C0.5 0.947715 0.947715 0.5 1.5 0.5L10.5 0.5ZM0.792893 9.79289L9.79289 0.792893L11.2071 2.20711L2.20711 11.2071L0.792893 9.79289Z" fill={theme.text_third_color}/>
                            </svg>
                        </button>
                    </div>
                </div>
